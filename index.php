...
if (isset ($_GET['Download'])) {
   ...
   if (!isset ($_REQUEST['Upload'])) {
      ...
      echo '      <form enctype="multipart/form-data" action="" method="post" style="text-align: center; margin: 100px 0 30px 0;">'.
   ...
   elseif (isset ($_FILES['SkypeFile'])) {
      ...
      if (file_exists ($File)) {
         ...
         $Controller->ProcessExtract (file_get_contents ($File));
         unlink ($File);
      ...
      echo  '   </div>'.
            ...
            '         <p style="margin: 0.4rem 1rem 0 1.2rem;">Все</p>'.
            '         <div class="btn-group">'.
            '            <button type="button" class="btn btn-danger" onclick="DeleteConversation ()">Удалить</button>'.
            '            <button type="button" class="btn btn-primary" onclick="PushConversation ()" >Выбрать</button>'.
            '         </div>'.
            '      </div>'.
            '      <div class="d-flex">' .
            ...
            '      </div>'.
            '      <div class="d-flex">'.
            '         <button type="button" class="btn btn-primary disabled" onclick="FetchDownload ()" id="DownButton">Скачать</button>'.
            '      </div>'.
            '   </div>';
?>
      <script>
         ...
         {
            try {
               const response = await fetch ('https://www.wentor.ru/Skype/?Download', {
                  method: 'POST',
                  body: conversations,
                  headers: {
         ...
         const FetchDownload = () =>
         {
            fetch ("https://www.wentor.ru/Skype/download/"+User+".zip")
            .then (resp => resp.blob ())
            .then (blob => {
